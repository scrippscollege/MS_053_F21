...
use Poetic::Violence;
# Software
for the aggressive assault on society.
...
sub bought_off {
  ...
  for ($me = 0; $me <= SATISFIED; $me += EQUALITY_FOR_ALL) {
    $Exploit
      =
      push(@poverty_on_someone_else, $self - > {
        gain
      });
    die "poor"
    if $Exploit
...
# Perl Routines
for the redistribution of the world 's wealth
...
foreach my $RichBastardIndex(keys % {
  The_Rich
}) {
  &
  ReDisdributeCash(\ % {
...
sub ReDisdributeCash {
  ...
  while ($RichBastard_REFERENCE - > {
      CASH
    } >= TO_MUCH) {
    foreach my $Index(keys @ {
      Poor
    }) {
      $RichBastard_REFERENCE - > {
        CASH
      }—;
      $Poor - > {
        $Index
